...
static  void tpd_up(int x, int y,int *count) {
 ...
 static int tpd_touchinfo(struct touch_info *cinfo, struct touch_info *pinfo)
 {
	...
	i2c_smbus_read_i2c_block_data(i2c_client, 0x00, 3, &(data[0]));
	/* Device Mode[2:0] == 0 :Normal operating Mode*/
	if((data[0] & 0x70) != 0) return false; 
	/*get the number of the touch points*/
	point_num= data[2] & 0x0f;
	TPD_DEBUG("point_num =%d\n",point_num);
	if(point_num == 0) return true;

	i2c_smbus_read_i2c_block_data(i2c_client, 0x03, 30, &(data[0]));
	
//	TPD_DEBUG("received raw data from touch panel as following:\n");
//	TPD_DEBUG("[data[0]=0x%x,data[1]=0x%x ,data[2]=0x%x ,data[3]=0x%x ,data[4]=0x%x ,data[5]=0x%x\n",data[0],data[1],data[2],data[3],data[4],data[5]);
//	TPD_DEBUG("[data[6]=0x%x,data[7]=0x%x ,data[8]=0x%x ,data[9]=0x%x ,data[10]=0x%x ,data[11]=0x%x\n",data[6],data[7],data[8],data[9],data[10],data[11]);
//	TPD_DEBUG("[data[12]=0x%x,data[13]=0x%x ,data[14]=0x%x ,data[15]=0x%x ,data[16]=0x%x ,data[17]=0x%x\n",data[12],data[13],data[14],data[15],data[16],data[17]);
//	TPD_DEBUG("[data[18]=0x%x,data[19]=0x%x ,data[20]=0x%x ,data[21]=0x%x ,data[22]=0x%x ,data[23]=0x%x\n",data[18],data[19],data[20],data[21],data[22],data[23]);
//	TPD_DEBUG("[data[24]=0x%x,data[25]=0x%x ,data[26]=0x%x ,data[27]=0x%x ,data[28]=0x%x ,data[29]=0x%x\n",data[24],data[25],data[26],data[27],data[28],data[29]);
...
//	   TPD_DEBUG("Procss raw data...\n");
		...
		{
			cinfo->p[i] = data[6*i] >> 6; //event flag 
                   cinfo->id[i] = data[6*i+2]>>4; //touch id
...
reset_proc:   
  ...
  mt_set_gpio_dir(GPIO_CTP_EN_PIN, GPIO_DIR_OUT);
	...
	{
		TPD_DMESG("I2C read device mode transfer error, line: %d\n", __LINE__);
	...
	TPD_DEBUG("FT5206 driver probe, device mode=0x%x\n",data);


	if((i2c_smbus_read_i2c_block_data(i2c_client, 0xa6, 1, &data))<0)
	{
		TPD_DMESG("I2C read FW version transfer error, line: %d\n", __LINE__);
#ifdef TPD_RESET_ISSUE_WORKAROUND
        if ( reset_count < TPD_MAX_RESET_COUNT )
        {
            reset_count++;
	...
	TPD_DEBUG("FT5206 driver probe, FW version=0x%x\n",data);
