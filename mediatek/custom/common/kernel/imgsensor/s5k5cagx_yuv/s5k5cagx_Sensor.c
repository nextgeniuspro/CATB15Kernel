 ...
 * [ALPS00358276] [blocking][基本功能][CMCC Case Fail][TS-NATIVEFUNC-PHOTOCAMERA-SET-000006][M]不支持ISO感光度、对比度、饱和度调节，人脸识别。
 * .
...
{    
    ...
    SENSORDB("S5K5CAGX_set_contrast is %d\n",iPara);
    switch(iPara)
    {
...
UINT32 S5K5CAGXYUVSensorSetting(FEATURE_ID iCmd, UINT16 iPara)
{
	...
	switch (iCmd) 
	{
      ...
      //S5K5CAGX_set_AE_mode(S5K5CAGX_AE_ENABLE);
		...
		break; 
		case FID_ISP_SAT:
			SENSORDB("S5K5CAGXYUVSensorSetting FID_ISP_SAT \n");
			S5K5CAGX_set_saturation(iPara);
		break;
	    case FID_ISP_CONTRAST:
	        SENSORDB("S5K5CAGXYUVSensorSetting FID_ISP_CONTRAST \n");
...
static void S5K5CAGXGetEvAwbRef(UINT32 pSensorAEAWBRefStruct/*PSENSOR_AE_AWB_REF_STRUCT Ref*/)
{
	...
	pRef->SensorAERef.AeRefLV05Shutter = 1503;
    pRef->SensorAERef.AeRefLV05Gain = 496 * 2; /* 7.75x, 128 base */
    pRef->SensorAERef.AeRefLV13Shutter = 49;
    pRef->SensorAERef.AeRefLV13Gain = 64 * 2; /* 1x, 128 base */
    pRef->SensorAwbGainRef.AwbRefD65Rgain = 188; /* 1.46875x, 128 base */
    pRef->SensorAwbGainRef.AwbRefD65Bgain = 128; /* 1x, 128 base */
    pRef->SensorAwbGainRef.AwbRefCWFRgain = 160; /* 1.25x, 128 base */
    pRef->SensorAwbGainRef.AwbRefCWFBgain = 164; /* 1.28125x, 128 base */
	SENSORDB("exit");
}
...
static void S5K5CAGXGetCurAeAwbInfo(UINT32 pSensorAEAWBCurStruct/*PSENSOR_AE_AWB_CUR_STRUCT Info*/)
{
    ...
    pInfo->SensorAECur.AeCurGain = S5K5CAGXReadGain() * 2; /* 128 base */   
    pInfo->SensorAwbGainCur.AwbCurRgain = S5K5CAGXReadAwbRGain()<< 1; /* 128 base */ 
    pInfo->SensorAwbGainCur.AwbCurBgain = S5K5CAGXReadAwbBGain()<< 1; /* 128 base */
	SENSORDB("exit");
}
