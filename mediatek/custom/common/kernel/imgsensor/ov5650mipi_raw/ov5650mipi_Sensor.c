...
MSDK_SCENARIO_ID_ENUM OV5650MIPI_CurrentScenarioId = ACDK_SCENARIO_ID_CAMERA_PREVIEW;
	...
	if(KAL_TRUE == OV5650MIPI_g_Fliker_Mode)
		{
			...
			SENSORDB("[OV5650MIPI_Write_Shutter]pv_clk:%d\n",pclk);
			SENSORDB("[OV5650MIPI_Write_Shutter]line_length:%d\n",line_length);
			//SENSORDB("[OV5650_Write_Shutter]frame_height:%d\n",frame_length);
			SENSORDB("[OV5650MIPI_Write_Shutter]framerate(10base):%d\n",realtime_fp);
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	...
	OV5650MIPI_CurrentScenarioId = ScenarioId;
...
UINT32 OV5650MIPISetVideoMode(UINT16 u2FrameRate)
{
  ...
  if((30 == u2FrameRate)||(15 == u2FrameRate))
  	{
  	    if( MSDK_SCENARIO_ID_CAMERA_ZSD)
  	    {
...
MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData=(MSDK_SENSOR_CONFIG_STRUCT *) pFeaturePara;
	...
	switch (FeatureId)
	{
		...
		case SENSOR_FEATURE_GET_PERIOD:
			switch(OV5650MIPI_CurrentScenarioId)
				{
					case MSDK_SCENARIO_ID_CAMERA_ZSD:
