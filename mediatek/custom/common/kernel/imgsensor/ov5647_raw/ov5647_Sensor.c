...
UINT32 OV5647SetMaxFrameRate(UINT16 u2FrameRate)
{
	...
	OV5647_TRACE("[OV5647SetMaxFrameRate]u2FrameRate=%d",u2FrameRate);
...
/*No Need to implement this function*/
	...
	OV5647_TRACE("[OV5647_night_mode]enable=%d",enable);
...
static void OV5647_camera_para_to_sensor(void)
{
    ...
    OV5647_TRACE("OV5647_camera_para_to_sensor\n");
...
static void OV5647_sensor_to_camera_para(void)
{
    ...
    OV5647_TRACE("OV5647_sensor_to_camera_para\n");
...
{
   OV5647_TRACE("OV5647_get_sensor_group_count\n");
...
{
   OV5647_TRACE("OV5647_get_sensor_group_info\n");
...
inline static void OV5647_get_sensor_item_info(MSDK_SENSOR_ITEM_INFO_STRUCT *para)
{
  ...
  OV5647_TRACE("OV5647_get_sensor_item_info\n");
...
inline static kal_bool OV5647_set_sensor_item_info(MSDK_SENSOR_ITEM_INFO_STRUCT *para)
{
  ...
  OV5647_TRACE("OV5647_set_sensor_item_info\n");
...
static void OV5647_Sensor_1M(void)
{
	...
	//26M MCLK 48.1M PCLK
	OV5647_write_cmos_sensor(0x0100, 0x00);
	//OV5647_write_cmos_sensor(0x4005, 0x18); // update BLC
	//OV5647_write_cmos_sensor(0x4005, 0x1A); // update BLC
			OV5647_write_cmos_sensor(0x4005, 0x18);//Upate BLC when gain changed
	OV5647_write_cmos_sensor(0x4051, 0x8F);
#ifdef OV5647_DRIVER_TRACE	
	OV5647_TRACE("Write Reg 0x4005 = 0x18");
#endif	
	...
	OV5647_write_cmos_sensor(0x380f, 0xd8); /*	TIMING VTS 984 */
		...
		OV5647_write_cmos_sensor(0x380c, 0x06); /*	TIMING HTS 0x065e */
		OV5647_write_cmos_sensor(0x380d, 0xFE); /*	TIMING HTS 1790 */
		OV5647_write_cmos_sensor(0x380e, 0x03); /*	TIMING VTS 0x03d8 */
		OV5647_write_cmos_sensor(0x380f, 0xd8); /*	TIMING VTS 984 */
#elif defined(__OV5647_54M__)
		OV5647_write_cmos_sensor(0x380c, 0x07); /*	TIMING HTS 0x073A */
		OV5647_write_cmos_sensor(0x380d, 0x3A); /*	TIMING HTS 1850 */
		OV5647_write_cmos_sensor(0x380e, 0x03); /*	TIMING VTS 0x03d8 */
		OV5647_write_cmos_sensor(0x380f, 0xd8); /*	TIMING VTS 984 */
	
#elif defined(__OV5647_56M__)
		OV5647_write_cmos_sensor(0x380c, 0x07); /*	TIMING HTS 0x0768 */
		OV5647_write_cmos_sensor(0x380d, 0x68); /*	TIMING HTS 1986 */
		OV5647_write_cmos_sensor(0x380e, 0x03); /*	TIMING VTS 0x03d8 */
		OV5647_write_cmos_sensor(0x380f, 0xd8); /*	TIMING VTS 984 */
#endif
...
static void OV5647_Sensor_5M(void)
{
...
#ifdef OV5647_DRIVER_TRACE	
	OV5647_TRACE("Write Reg 0x4005 = 0x1a");
#endif		
	...
	OV5647_write_cmos_sensor( 0x3a0d, 0x08); 
	OV5647_write_cmos_sensor( 0x3a0e, 0x06); 
	OV5647_write_cmos_sensor( 0x3503, 0x07);
	OV5647_write_cmos_sensor( 0x0100, 0x01); 
}
...
UINT32 OV5647Open(void)
{
	...
	OV5647_TRACE("OV5647Open, sensor_id:%x \n",sensor_id);
...
UINT32 OV5642GetSensorID(UINT32 *sensorID) 
{
	...
	*sensorID=((OV5647_read_cmos_sensor(0x300A) << 8) | OV5647_read_cmos_sensor(0x300B));	
	OV5647_TRACE("OV5647Get, sensor_id:%x \n",*sensorID);
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    OV5647_TRACE("[OV5647 Preview] target size=%d x %d, mirror=%d\n", \
						  ...
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    OV5647_TRACE("[OV5647 Capture] target size=%d x %d, ZoomFactor=%d\n", \
	...
	
	{
		...
		// For ZSD Scenario, Do not need to write shutter.
		if(CurrentScenarioId== MSDK_SCENARIO_ID_CAMERA_ZSD)
			return ERROR_NONE;
	...
	shutter = (shutter * (((kal_uint32)(pv_line_length * 806 * 1000))/((kal_uint32)(OV5647_sensor.line_length * 563))))/1000;
        ...
        OV5647_TRACE("[OV5647 capture]shutter=%x",shutter);
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
    ...
    case MSDK_SCENARIO_ID_CAMERA_ZSD:
        pSensorInfo->SensorPreviewResolutionX=OV5647_IMAGE_SENSOR_FULL_WIDTH;
	    pSensorInfo->SensorPreviewResolutionY=OV5647_IMAGE_SENSOR_FULL_HEIGHT;
...
UINT32 OV5647SetVideoMode(UINT16 u2FrameRate)
{
	...
	if(u2FrameRate == 30){
		...
		// TODO:
		//return TRUE;
		printk("[soso][OV5647SetVideoMode] error framerate!!! : u2FrameRate = %d", u2FrameRate);
		return TRUE;
	}
	...
	if((u2FrameRate == 30)&&(OV5647AutoFlicKerMode==KAL_TRUE))
		u2FrameRate = 296;
	if((u2FrameRate == 15)&&(OV5647AutoFlicKerMode==KAL_TRUE))
		u2FrameRate = 148;
	else
		u2FrameRate = 10 * u2FrameRate;
	
	OV5647SetMaxFrameRate(u2FrameRate);
	OV5647_Write_Shutter(OV5647_sensor.shutter);   // To solve Video issue
