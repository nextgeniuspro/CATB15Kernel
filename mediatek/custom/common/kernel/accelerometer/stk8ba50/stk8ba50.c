...
static struct i2c_board_info __initdata i2c_stk8ba50={ I2C_BOARD_INFO("STK8BA50", STK8BA50_I2C_SLAVE_ADDR)};
...
{
	struct stk8ba50_i2c_data *obj = i2c_get_clientdata(client);
	s8 ofs[STK8BA50_AXES_NUM] = {0x00, 0x00, 0x00};
	int err;
...
static int STK8BA50_ReadCalibration(struct i2c_client *client, int dat[STK8BA50_AXES_NUM])
{
    ...
    dat[obj->cvt.map[STK8BA50_AXIS_X]] = obj->cvt.sign[STK8BA50_AXIS_X]*(obj->offset[STK8BA50_AXIS_X]/mul);
    dat[obj->cvt.map[STK8BA50_AXIS_Y]] = obj->cvt.sign[STK8BA50_AXIS_Y]*(obj->offset[STK8BA50_AXIS_Y]/mul);
    dat[obj->cvt.map[STK8BA50_AXIS_Z]] = obj->cvt.sign[STK8BA50_AXIS_Z]*(obj->offset[STK8BA50_AXIS_Z]/mul);                        
...
static int STK8BA50_SetCali(struct i2c_client *client, char sstate)
{
	...
	STK8BA50_SetDelay(client, 13333);	/*	75 Hz ODR */	
...
static int STK8BA50_WriteCalibration(struct i2c_client *client, int dat[STK8BA50_AXES_NUM])
{
	...
	int lsb = stk8ba50_offset_resolution.sensitivity;
	u8 databuf[2]; 
	int res = 0;
	...
	obj->offset[STK8BA50_AXIS_X] = (s8)(obj->cvt.sign[STK8BA50_AXIS_X]*(cali[obj->cvt.map[STK8BA50_AXIS_X]])*(divisor));
	obj->offset[STK8BA50_AXIS_Y] = (s8)(obj->cvt.sign[STK8BA50_AXIS_Y]*(cali[obj->cvt.map[STK8BA50_AXIS_Y]])*(divisor));
	obj->offset[STK8BA50_AXIS_Z] = (s8)(obj->cvt.sign[STK8BA50_AXIS_Z]*(cali[obj->cvt.map[STK8BA50_AXIS_Z]])*(divisor));

	/*convert software calibration using standard calibration*/
	obj->cali_sw[STK8BA50_AXIS_X] = obj->cvt.sign[STK8BA50_AXIS_X]*(cali[obj->cvt.map[STK8BA50_AXIS_X]])%(divisor);
	obj->cali_sw[STK8BA50_AXIS_Y] = obj->cvt.sign[STK8BA50_AXIS_Y]*(cali[obj->cvt.map[STK8BA50_AXIS_Y]])%(divisor);
	obj->cali_sw[STK8BA50_AXIS_Z] = obj->cvt.sign[STK8BA50_AXIS_Z]*(cali[obj->cvt.map[STK8BA50_AXIS_Z]])%(divisor);
    ...
    STK8BA50_SetPowerMode(obj->client,false);
	if(err = hwmsen_write_block(obj->client, STK8BA50_OFSTFILTX, obj->offset, STK8BA50_AXES_NUM))
	{
...
static int STK8BA50_Init_client(struct i2c_client *client, int reset_cali)
{
	...
	result = STK8BA50_SetPowerMode(client,false);
	...
	res = STK8BA50_SetBWSEL(client, 0x0);
	if(res != STK8BA50_SUCCESS)
	{
...
static int STK8BA50_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	else
	{
		...
		//Out put the mg
		acc[STK8BA50_AXIS_X] = acc[STK8BA50_AXIS_X] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
		acc[STK8BA50_AXIS_Y] = acc[STK8BA50_AXIS_Y] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
		acc[STK8BA50_AXIS_Z] = acc[STK8BA50_AXIS_Z] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;		
	...
	switch(cmd)
	{
		...
		case GSENSOR_IOCTL_SET_CALI:
			...
			{   
				...
				cali[STK8BA50_AXIS_X] = sensor_data.x * obj->reso->sensitivity / GRAVITY_EARTH_1000;
				cali[STK8BA50_AXIS_Y] = sensor_data.y * obj->reso->sensitivity / GRAVITY_EARTH_1000;
				cali[STK8BA50_AXIS_Z] = sensor_data.z * obj->reso->sensitivity / GRAVITY_EARTH_1000;			  
		...
		case GSENSOR_IOCTL_GET_CALI:
			...
			sensor_data.x = cali[STK8BA50_AXIS_X] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
			sensor_data.y = cali[STK8BA50_AXIS_Y] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
			sensor_data.z = cali[STK8BA50_AXIS_Z] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
...
static int stk8ba50_resume(struct i2c_client *client)
{
	...
	if(err = STK8BA50_Init_client(client, 0))
	{
...
static void stk8ba50_late_resume(struct early_suspend *h)
{
	...
	STK8BA50_power(obj->hw, 1);
	if(err = STK8BA50_Init_client(obj->client, 0))
	{
		GSE_ERR("initialize client fail!!\n");
		return;        
	}
...
static int stk8ba50_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(err = stk8ba50_create_attr(&stk8ba50_gsensor_driver.driver))
	{
...
static int stk8ba50_i2c_remove(struct i2c_client *client)
{
	...
	if(err = stk8ba50_delete_attr(&stk8ba50_gsensor_driver.driver))
	{
...
{
	GSE_FUN();
	i2c_register_board_info(0, &i2c_stk8ba50, 1);
	if(platform_driver_register(&stk8ba50_gsensor_driver))
	{
...
{
	GSE_FUN();
	platform_driver_unregister(&stk8ba50_gsensor_driver);
}
/*----------------------------------------------------------------------------*/
