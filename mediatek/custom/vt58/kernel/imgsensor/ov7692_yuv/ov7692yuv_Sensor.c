/*************************************
 * For VT58 sub camera sensor driver *
 *************************************/
/* Copyright Statement:
 *
...
//#define OV7692YUV_USE_LENS_SHADING_CORRECTION  //Non-use now.
/*Since quality issue, OV7692 fixed 15 fps from initial*/
//#define OV7692YUV_USE_DYNAMIC_FRAME_RATE       //Non-use now.
...
{
  kal_uint8 Reg = 0x16;
  //printk("***Walter-debug: Enter @%s OV7692Sensor.Mirror = %d, Mirror = %d \n" ,__func__, OV7692Sensor.Mirror, Mirror);
  ...
  {
//Walter-add-start for sub camera rotation
  case IMAGE_NORMAL:
    break;
//Walter-add-end for sub camera rotation
  case IMAGE_H_MIRROR:
    Reg |= 0x40;
    break;
  case IMAGE_V_MIRROR:
...
#ifdef OV7692YUV_USE_LENS_SHADING_CORRECTION
/*************************************************************************
* FUNCTION
*    OV7692LSCSetting
*
...
static void OV7692SetDummy(kal_uint16 DummyPixel, kal_uint16 DummyLine)
{
  ...
  if (update)
  {
    ...
    OV7692_write_cmos_sensor(0x50, 0x99); //BandStep50 /* 50Hz banding step */
    OV7692_write_cmos_sensor(0x51, 0x7F); //BandStep60 /* 60Hz banding step */
    ...
    OV7692_write_cmos_sensor(0x21, 0x23); //((MaxBand50&0x0F) << 4)|(MaxBand60&0x0F)
  }
}
...
static void OV7692InitialSetting(void)
{
   ...
   */
   ...
   /*
    * Solution from vendor OV to solve the short green screen when
    * enabling the sub-camera
    *    Can adding below setting in the end of initial,
    *    78 13 f5
    *    78 01 b8
    *    78 02 8d
    *    78 03 80
    *    Then wait 100msec delay by backend then set 0x13=f7
    *
    * NOTE THAT THIS PATCH DOES NOT WORK CURRENTLY. WAITING FOR VENDOR'S
    * OTHER SOLUTION.
    */
#if 0
   SENSORDB("[OV7692] Applying the patch for short green screen by vendor\n");
   OV7692_write_cmos_sensor(0x13,0xF5);
   OV7692_write_cmos_sensor(0x01,0xB8);
   OV7692_write_cmos_sensor(0x02,0x8D);
   OV7692_write_cmos_sensor(0x03,0x80);
   msleep(100);
   OV7692_write_cmos_sensor(0x13,0xF7);
#endif
/*[VT58][BugID:456] S Mark sync HA05.V003 initial setting*/
#ifdef ORG_VER
   //SENSORDB("[OV7692][Mark] Original init setting\n");
   OV7692_write_cmos_sensor(0x12,0x80);
   msleep(5);
   OV7692_write_cmos_sensor(0x0c,0xd6);
   ...
   OV7692_write_cmos_sensor(0x2c,0x00);
   OV7692_write_cmos_sensor(0x20,0x00);
   OV7692_write_cmos_sensor(0x21,0x57);
   OV7692_write_cmos_sensor(0x50,0x4d);
   ...
   OV7692_write_cmos_sensor(0x85,0x00);
   OV7692_write_cmos_sensor(0x86,0x00);
   OV7692_write_cmos_sensor(0x87,0x00);
   OV7692_write_cmos_sensor(0x88,0x00);
   OV7692_write_cmos_sensor(0x89,0x19);
   OV7692_write_cmos_sensor(0x8a,0x11);
   OV7692_write_cmos_sensor(0x8b,0x11);
                                       
   OV7692_write_cmos_sensor(0xbb,0xac);
   OV7692_write_cmos_sensor(0xbc,0xae);
   OV7692_write_cmos_sensor(0xbd,0x02);
   OV7692_write_cmos_sensor(0xbe,0x1f);
   OV7692_write_cmos_sensor(0xbf,0x93);
   OV7692_write_cmos_sensor(0xc0,0xb1);
   OV7692_write_cmos_sensor(0xc1,0x1a);
   ...
   OV7692_write_cmos_sensor(0x5A,0x10);
   OV7692_write_cmos_sensor(0x5B,0xa1);
   OV7692_write_cmos_sensor(0x5C,0x3a);
   ...
   OV7692_write_cmos_sensor(0x92,0x94);
   OV7692_write_cmos_sensor(0x93,0x8e);
   OV7692_write_cmos_sensor(0x94,0x18);
   OV7692_write_cmos_sensor(0x95,0x1b);
   OV7692_write_cmos_sensor(0x96,0xf0);
   OV7692_write_cmos_sensor(0x97,0x10);
   OV7692_write_cmos_sensor(0x98,0x7a);
   OV7692_write_cmos_sensor(0x99,0x75);
   OV7692_write_cmos_sensor(0x9a,0x92);
   OV7692_write_cmos_sensor(0x9b,0x8f);
   ...
   OV7692_write_cmos_sensor(0xa0,0xc0);
   OV7692_write_cmos_sensor(0xa1,0xb1);
   OV7692_write_cmos_sensor(0xa2,0x1c);
                                       
   OV7692_write_cmos_sensor(0x14,0x2b);
   OV7692_write_cmos_sensor(0xd2,0x06);
#else
   //SENSORDB("[OV7692][Mark] Use HA05.V003 init setting\n");
   OV7692_write_cmos_sensor(0x12,0x80);
   msleep(5);
   OV7692_write_cmos_sensor(0x0c,0xd6);
   OV7692_write_cmos_sensor(0x0e,0x08);
   ...
   OV7692_write_cmos_sensor(0x2c,0x00);
   OV7692_write_cmos_sensor(0x20,0x00);
   OV7692_write_cmos_sensor(0x21,0x23); //0x57
   OV7692_write_cmos_sensor(0x50,0x99); //0x4d
   OV7692_write_cmos_sensor(0x51,0x7F); //0x40
   OV7692_write_cmos_sensor(0x4c,0x7d);
   OV7692_write_cmos_sensor(0x0e,0x00);
   OV7692_write_cmos_sensor(0x80,0x7f);
   OV7692_write_cmos_sensor(0x15,0x00);
   ...
   OV7692_write_cmos_sensor(0x87,0x00);
   OV7692_write_cmos_sensor(0x88,0x00);
   OV7692_write_cmos_sensor(0x89,0x13); //0x19
   OV7692_write_cmos_sensor(0x8a,0x11);
   OV7692_write_cmos_sensor(0x8b,0x11);
   ...
   OV7692_write_cmos_sensor(0x5B,0xa1);
   OV7692_write_cmos_sensor(0x5C,0x3a);
   OV7692_write_cmos_sensor(0x5d,0x22);
   OV7692_write_cmos_sensor(0x24,0x88);
   OV7692_write_cmos_sensor(0x25,0x78);
   OV7692_write_cmos_sensor(0x26,0xb3);
   OV7692_write_cmos_sensor(0xa3,0x05);
   ...
   OV7692_write_cmos_sensor(0xa1,0xb1);
   OV7692_write_cmos_sensor(0xa2,0x1c);
   OV7692_write_cmos_sensor(0x00,0xF0);                                    
   OV7692_write_cmos_sensor(0x14,0x3b); //0x2b
   OV7692_write_cmos_sensor(0xd2,0x06);

//;gamma_1                                
   OV7692_write_cmos_sensor(0xa3,0x10); 
   OV7692_write_cmos_sensor(0xa4,0x1c); 
   OV7692_write_cmos_sensor(0xa5,0x34); 
   OV7692_write_cmos_sensor(0xa6,0x5d); 
   OV7692_write_cmos_sensor(0xa7,0x6a); 
   OV7692_write_cmos_sensor(0xa8,0x76); 
   OV7692_write_cmos_sensor(0xa9,0x7f); 
   OV7692_write_cmos_sensor(0xaa,0x88); 
   OV7692_write_cmos_sensor(0xab,0x8f); 
   OV7692_write_cmos_sensor(0xac,0x96); 
   OV7692_write_cmos_sensor(0xad,0xa3); 
   OV7692_write_cmos_sensor(0xae,0xaf); 
   OV7692_write_cmos_sensor(0xaf,0xc4); 
   OV7692_write_cmos_sensor(0xb0,0xd7); 
   OV7692_write_cmos_sensor(0xb1,0xe8); 
   OV7692_write_cmos_sensor(0xb2,0x20); 
#endif
/*[VT58][BugID:456] E Mark sync HA05.V003 initial setting*/
...
//Walter-modify-start for get correct sensor ID.
UINT32 OV7692GetSensorID(UINT32 *sensorID) 
{
kal_uint16 sensor_id = 0;
sensor_id=((OV7692_read_cmos_sensor(0x0A)<< 8)|OV7692_read_cmos_sensor(0x0B));
	if (sensor_id != OV7692_SENSOR_ID) {
...
*sensorID = sensor_id;
		SENSORDB("OV7692Open sensor_id is %x\n", (unsigned int)*sensorID );
    return ERROR_NONE;
}
//Walter-modify-end for get correct sensor ID.
...
#ifdef OV7692YUV_USE_DYNAMIC_FRAME_RATE
	OV7692_FPS(30);
#endif
//Walter-add-start for sub camera rotation
...
//Walter-add-end for sub camera rotation
	...
	OV7692_write_cmos_sensor(0x14,0x3b );//0x21//for banding 50HZ
...
#else
   SENSORDB("For OV7692YUV, frame rate is fixed at 15 fps, will no change!\n");
#endif
...
kal_uint32 OV7692GetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution)
{
    ...
    pSensorResolution->SensorPreviewWidth=OV7692_IMAGE_SENSOR_WIDTH_DRV - 2;
	pSensorResolution->SensorPreviewHeight=OV7692_IMAGE_SENSOR_HEIGHT_DRV - 2;
	return ERROR_NONE;
}	/* OV7692GetResolution() */
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	...
	switch (ScenarioId)
	{
		...
		case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		case MSDK_SCENARIO_ID_CAMERA_CAPTURE_MEM:
			OV7692_Capture(pImageWindow, pSensorConfigData);
		break;
		default:
...
kal_uint8  temp_reg;
    ...
    // {0x40,0xA0}, /* TUNGSTEN */
    //};
//printk("***Walter-debug: Enter @%s \n" ,__func__);
	if (OV7692Sensor.Wb == para)
    {
      return TRUE;
    }
    OV7692Sensor.Wb = para;
temp_reg=OV7692_read_cmos_sensor(0x13);
	switch (para)
	{
		case AWB_MODE_AUTO:
		//printk("***Walter-debug: Enter @%s AWB_MODE_AUTO \n" ,__func__);
OV7692_write_cmos_sensor(0x13,temp_reg|0x2);
			//OV7692AwbEnable(KAL_TRUE);	
			break;
		case AWB_MODE_DAYLIGHT: //sunny
		//printk("***Walter-debug: Enter @%s WB_MODE_DAYLIGHT \n" ,__func__);
			OV7692_write_cmos_sensor(0x13,temp_reg&~0x2);  // Disable AWB 
			OV7692_write_cmos_sensor(0x01, 0x8A); 
			OV7692_write_cmos_sensor(0x02, 0xA7); 
			OV7692_write_cmos_sensor(0x03, 0x80);
			//i = 1;
			break;
		case AWB_MODE_CLOUDY_DAYLIGHT: //cloudy
		//printk("***Walter-debug: Enter @%s AWB_MODE_CLOUDY_DAYLIGHT \n" ,__func__);
			OV7692_write_cmos_sensor(0x13,temp_reg&~0x2);  // Disable AWB 
			OV7692_write_cmos_sensor(0x01, 0x7A); 
			OV7692_write_cmos_sensor(0x02, 0xA7); 
			OV7692_write_cmos_sensor(0x03, 0x80);
			//i = 0;
			break;
		case AWB_MODE_INCANDESCENT: //office 
		//printk("***Walter-debug: Enter @%s AWB_MODE_INCANDESCENT \n" ,__func__);
			OV7692_write_cmos_sensor(0x13,temp_reg&~0x2);  // Disable AWB 
			OV7692_write_cmos_sensor(0x01, 0xB7); 
			OV7692_write_cmos_sensor(0x02, 0x6A); 
			OV7692_write_cmos_sensor(0x03, 0x80);
			//i = 2;			
			break;
		case AWB_MODE_TUNGSTEN: //home 
		//printk("***Walter-debug: Enter @%s AWB_MODE_TUNGSTEN \n" ,__func__);
			OV7692_write_cmos_sensor(0x13,temp_reg&~0x2);  // Disable AWB 
			OV7692_write_cmos_sensor(0x01, 0xC6); 
			OV7692_write_cmos_sensor(0x02, 0x70); 
			OV7692_write_cmos_sensor(0x03, 0x80);
			//i = 4;
			break;
		case AWB_MODE_FLUORESCENT: 
		//printk("***Walter-debug: Enter @%s AWB_MODE_FLUORESCENT \n" ,__func__);
			OV7692_write_cmos_sensor(0x13,temp_reg&~0x2);  // Disable AWB 
			OV7692_write_cmos_sensor(0x01, 0x87); 
			OV7692_write_cmos_sensor(0x02, 0x8E); 
			OV7692_write_cmos_sensor(0x03, 0x80);
			//i = 3;
			break; 
		default:
		//printk("***Walter-debug: Enter @%s default \n" ,__func__);
			return FALSE;
	}
	//if (i < 5) {
...
static BOOL OV7692_set_param_banding(UINT16 para)
{
	...
	switch (para)
	{
		case AE_FLICKER_MODE_50HZ:
			OV7692_write_cmos_sensor(0x14, 0x3b);//0x21
			break;
		case AE_FLICKER_MODE_60HZ:
			OV7692_write_cmos_sensor(0x14, 0x3a);//0x20
			break;
		default:
			return FALSE;
	}
...
static BOOL OV7692_set_param_exposure(UINT16 para)
{
	...
	switch (para)
	{
		...
		case AE_EV_COMP_10:// EV compensate 1.0
			...
			OV7692_write_cmos_sensor(0x24, 0x88); 
			OV7692_write_cmos_sensor(0x25, 0x78); 
			OV7692_write_cmos_sensor(0x26, 0xC4);
			break;
		case AE_EV_COMP_15:// EV compensate 1.5
			i = 1;
			break;
		case AE_EV_COMP_20:// EV compensate 2.0
			OV7692_write_cmos_sensor(0x24, 0x98); 
			OV7692_write_cmos_sensor(0x25, 0x88); 
			OV7692_write_cmos_sensor(0x26, 0xD5);
			//i = 0;
			break;
		...
		case AE_EV_COMP_n10:// EV compensate -1.0
			OV7692_write_cmos_sensor(0x24, 0x68); 
			OV7692_write_cmos_sensor(0x25, 0x58); 
			OV7692_write_cmos_sensor(0x26, 0xA2);
			//i = 6;
			break;
		...
		case AE_EV_COMP_n20:// EV compensate -2.0
			OV7692_write_cmos_sensor(0x24, 0x58); 
			OV7692_write_cmos_sensor(0x25, 0x48); 
			OV7692_write_cmos_sensor(0x26, 0x91);
			//i = 8;
			break;
		default:
			return FALSE;
...
static kal_uint32 OV7692_YUVSensorSetting(FEATURE_ID iCmd, UINT16 iPara)
{
	...
	switch (iCmd) 
	{
		case FID_SCENE_MODE:
		    ...
		    break; 
		case FID_AWB_MODE:
		//printk("***Walter-debug: Enter @%s FID_AWB_MODE \n" ,__func__);
...
//Walter-modify-start for get correct sensor ID.
		case SENSOR_FEATURE_CHECK_SENSOR_ID:
            OV7692GetSensorID(pFeatureReturnPara32); 
			break;	
//Walter-modify-end for get correct sensor ID.
		default:
			break;
