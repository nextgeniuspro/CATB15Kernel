...
extern int DumpNativeInfo(void);
...
void aee_dumpbasic(void)
{
	...
	/* Dump native process registers and related information */
...
void aee_kernel_exception_api(const char *file, const int line, const int db_opt, const char *module, const char *msg, ...)
{
	...
	if(g_aee_api && g_aee_api->kernel_exception) {
		char *msgbuf = kmalloc(KERNEL_REPORT_LENGTH, GFP_ATOMIC);
		char *msgbuf2 = kmalloc(KERNEL_REPORT_LENGTH,GFP_ATOMIC);
