...
int BAT_CheckBatteryStatus(void)
{
    ...
    else
    {
        ...
        cci_fg_volt= FGADC_Get_FG_Voltage();
...
int bat_thread_kthread(void *x)
{
    ...
    while (1) {               
        ...
        BAT_thread();
        #else
		//move fgauge_initialization to pmic probe
        if(g_FG_init == 1)
        {
            g_FG_init=2;
