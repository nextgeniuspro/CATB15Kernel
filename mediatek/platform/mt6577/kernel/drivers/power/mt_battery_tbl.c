...
#if 0	
       //Set SPM = 0, 
       mt_set_gpio_mode(GPIO_SPM, GPIO_MODE_00);
       mt_set_gpio_dir(GPIO_SPM, GPIO_DIR_OUT);
       mt_set_gpio_out(GPIO_SPM, GPIO_OUT_ZERO);
#endif	   
}
#endif
		    ...
		    pchr_turn_off_charging_ncp1851();
              ...
              else
              {
                  if((gFG_booting_counter_I_FLAG == 2) || (g_ocv_lookup_done == 1))
                  {
                      ...
                      mt_set_gpio_mode(GPIO_SPM, GPIO_MODE_00);
                      mt_set_gpio_dir(GPIO_SPM, GPIO_DIR_OUT);
                      mt_set_gpio_out(GPIO_SPM, GPIO_OUT_ONE);
                      if (Enable_BATDRV_LOG == 1) {
