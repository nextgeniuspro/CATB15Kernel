...
//Jeffrey_Huang, Bug416 - Resolve MMI show issue for headset detection, Start
static int cable_type_saved = 0;
//Jeffrey_Huang, Bug416 - Resolve MMI show issue for headset detection, End
...
void accdet_work_callback(struct work_struct *work)
{
    ...
    {
		//Jeffrey_Huang, Bug416 - Resolve MMI show issue for headset detection, Start 
		if(cable_type == HEADSET_MIC && cable_type_saved == HEADSET_NO_MIC) 
		{ 
			switch_set_state((struct switch_dev *)&accdet_data, NO_DEVICE); 
			msleep(10); 
			ACCDET_DEBUG( " [accdet] cable_type == HEADSET_MIC\n");
		}
		cable_type_saved = cable_type;
		//Jeffrey_Huang, Bug416 - Resolve MMI show issue for headset detection, End  
