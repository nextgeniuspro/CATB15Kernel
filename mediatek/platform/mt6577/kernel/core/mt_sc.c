...
/*Bug90 Hsipo modify for power debug mechanism start*/
extern unsigned int suspend_info_enable;
static unsigned int i=0, status, index;
extern eint_func EINT_FUNC;
char cci_wakeup_reason[10][128];
unsigned int is_wakeup_reason_valid = 0;
/*Bug90 Hsipo modify for power debug mechanism end*/
static void sc_output_wake_reason(u32 wakesrc, u32 wakesta, u32 isr, u32 pause)
{
	...
	if(suspend_info_enable){
		if(i == 10)
			i=0;
		sprintf(&cci_wakeup_reason[i],"(%d-%02d-%02d %02d:%02d:%02d.%09lu UTC)%s(0x%x)",
				tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
				tm.tm_hour, tm.tm_min, tm.tm_sec, ts.tv_nsec,str,wakesta);
		if (eint_wake){
			status = readl(EINT_STA);
			sprintf(eint_sta_str," EINT_STA=0x%x",status);
			for (index = 0; index < EINT_MAX_CHANNEL; index++) {
				if (status & (1 << (index)))
					sprintf(cb_str," cb:%pf",EINT_FUNC.eint_func[index]);
			}
			strcat(cci_wakeup_reason[i], eint_sta_str);
			strcat(cci_wakeup_reason[i], cb_str);
		}
		i++;
	is_wakeup_reason_valid=1;
	}
	/*Bug90 Hsipo modify for power debug mechanism end*/
