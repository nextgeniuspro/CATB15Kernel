...
void mt_fixup(struct tag *tags, char **cmdline, struct meminfo *mi)
{
	...
	for (; temp_tags->hdr.size; temp_tags = tag_next(temp_tags))
	{
		...
		{
			//paul start
			setup_cci_project_id(temp_tags->u.cmdline.cmdline);
			setup_cci_emcp_id(temp_tags->u.cmdline.cmdline);
		    setup_cci_project(temp_tags->u.cmdline.cmdline);	
		    setup_cci_boot_reason(temp_tags->u.cmdline.cmdline);
//add for check HW ID2
			//setup_HWID2_check(temp_tags->u.cmdline.cmdline);
//add for check HW ID2
			//paul end
