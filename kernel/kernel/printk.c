...
//add for log timestamp
#include <linux/rtc.h>
int printk_timestamp(char* timetime)
{
	...
	sprintf(timetime,"%02d-%02d %02d:%02d:%02d.%06d",
	tm_android.tm_mon + 1, tm_android.tm_mday,
...
*/
				...
				char timestring[64];
				if(t>0){
					printk_timestamp(timestring);
					tlen = sprintf(tbuf, "[%s]",timestring);
				}else{
					tlen = sprintf(tbuf, "[0.00000]");
				}
//add for log timestamp
